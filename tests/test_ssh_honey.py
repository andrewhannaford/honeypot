...
class TestSSHServer(unittest.TestCase):
    ...
    def test_exec_request_logged_as_exec_attempt(self, mock_send_alert, mock_log_event):
        # This will fail initially because check_channel_exec_request is not implemented
    ...
    @patch("services.ssh_honey.log_event")
    @patch("services.ssh_honey.send_alert")
    def test_exec_request_with_url_logged_as_download_attempt(self, mock_send_alert, mock_log_event):
        if not hasattr(self.server, "check_channel_exec_request"):
            self.fail("check_channel_exec_request not implemented")
